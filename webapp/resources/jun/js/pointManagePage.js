$(document).ready(function(){
    ...
    const radioButtons = document.querySelectorAll('input[name="pointOpt"]');
    ...
    radioButtons.forEach(radio => {
        radio.addEventListener('change', function() {
            if (document.getElementById('directInput').checked) {
                directAmountInput.readOnly = false;
                directAmountInput.value = '';
            } else {
                directAmountInput.readOnly = true;
                directAmountInput.value = this.value;
            }
        });
    });
